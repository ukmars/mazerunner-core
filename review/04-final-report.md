...
|---|---|---|---|---|---|
| FR-01 | Navigation | `BLOCKED` heading (value 99) aliased to `LEFT` by `(99 - m_heading) & 0x3`; walled-in robot spins indefinitely, no diagnostic | **HIGH** | `mouse.h:426-427` | Check `newHeading == BLOCKED` before computing `hdgChange`; stop and emit error |
...
| FR-21 | Control | `Mouse::panic()` is defined but never called; no code path reaches it; intended as a safe-state handler | **MEDIUM** | `mouse.h:708-714` | Either wire `panic()` to the BLOCKED/error paths, or remove it |
